...
import { ScConfirmationDialog } from './confirmation-dialog';

@Injectable({
  providedIn: 'root',
})
export class ScConfirmationDialogTrigger {
  readonly dialog = inject(Dialog);

  async open(title: string, description: string, action: string): Promise<boolean> {
    const dialogRef = this.dialog.open<boolean>(ScConfirmationDialog, {
      minWidth: '300px',
